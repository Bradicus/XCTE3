...
module XCTEHtml
  class PageUtil
    ...
    def get_page_footer(colCount, listVarName)
      tFoot = HtmlNode.new("tfoot")
      tFoot.add_attribute("*ngIf", listVarName + "().pageCount")

      tFoot.add_child(make_paging_control(colCount, listVarName))
    end

    def make_paging_control(colCount, listVarName)
      ...
      li = HtmlNode.new("li")
                   .add_attribute("*ngFor", "let item of [].constructor(" + listVarName + "().pageCount);let i = index")
                   .add_class("page-item")
      ...
      lastPage = make_paging_button("&gt;&gt;", "goToPage(" + listVarName + "().pageCount - 1)")
