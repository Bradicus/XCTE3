...
class XCTECpp::MethodOperatorPlusEquals < XCTEPlugin

  def initialize
    @name = "method_operator_plus_equals"
    @language = "cpp"
  ...
  def get_definition(codeClass, cfg)
    ...
    eqString << "/**\n* Adds a " << codeClass.name << " to this one\n*/\n"
    eqString << "const " << codeClass.name
...
XCTEPlugin::registerPlugin(XCTECpp::MethodOperatorPlusEquals.new)
