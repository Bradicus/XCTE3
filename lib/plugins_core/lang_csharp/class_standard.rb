...
class XCTECSharp::ClassStandard < XCTEPlugin
  ...
  def genFileContent(codeClass, cfg, codeGen)
    ...
    classDec = codeClass.visibility + " class " + codeClass.name
    ...
    for var in varArray
      if var.elementId == CodeElem::ELEM_VARIABLE
        codeGen.add(XCTECSharp::Utils::getVarDec(var))
      elsif var.elementId == CodeElem::ELEM_COMMENT
        codeGen.sameLine(XCTECSharp::Utils::getComment(var))
    ...
    for fun in codeClass.functionSection
      if fun.elementId == CodeElem::ELEM_FUNCTION
        if fun.isTemplate
          ...
          if templ != nil
            codeGen.add(templ.get_definition(codeClass, cfg))
          else
          ...
          if templ != nil
            codeGen.add(templ.get_definition(fun, cfg))
          else
            #puts 'ERROR no plugin for function: ' + fun.name + '   language: csharp'
          end
        end
